...
const { normalizeRepeatedFailureKey } = require('./advisor-failure-tracker.js');
const {
  loadRouteConfig,
  resolveRoute,
...
function evaluateGatePolicy(rawEvent, options = {}) {
  ...
  if (rule.gateAction === 'human-approval') {
    ...
    if (decisionPacket.gateAction === 'hard-stop') return auditGateResult(decisionPacket, options);
