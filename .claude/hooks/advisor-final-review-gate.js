...
function evaluateFinalReviewGate(event = {}, options = {}) {
  ...
  let budgetStatus;
  try {
    budgetStatus = evaluateBudget({ eventType: 'advisor_final_review', correlationKey: event.correlationKey, taskId: event.taskId, sessionId: event.sessionId }, { ...options, root });
  } catch (error) {
  ...
  if (!freshness.fresh) {
    ...
    return auditFinalReviewResult(withBudget(strictMode
      ? block(
  ...
  if (!verdictRead.ok) return auditFinalReviewResult(withBudget(strictMode ? block('missing-final-verdict', 'Final advisor verdict artifact is missing or unreadable.', { freshness }) : advisoryOnly('missing-final-verdict', 'Final advisor verdict artifact is recommended before completion.', { freshness })), event, options);
  ...
  if (!evidenceRead.ok) return auditFinalReviewResult(withBudget(strictMode ? block('missing-verification-evidence', 'Verification evidence artifact is missing or unreadable.', { freshness }) : advisoryOnly('missing-verification-evidence', 'Verification evidence is recommended before completion.', { freshness })), event, options);
  ...
  if (verdict.status === 'PASS') return auditFinalReviewResult(withBudget(allow('fresh-pass-final-review', { freshness })), event, options);
  ...
  if (!decisionRead.ok) return auditFinalReviewResult(withBudget(strictMode ? block('missing-executor-decision', 'Non-PASS final verdict requires a matching executor-decision artifact.', { freshness }) : advisoryOnly('missing-executor-decision', 'Executor decision artifact is recommended before completion.', { freshness })), event, options);
  ...
  return auditFinalReviewResult(withBudget(allow('fresh-non-pass-final-review-with-executor-decision', { freshness })), event, options);
}
