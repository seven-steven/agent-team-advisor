...
async function runConformance(input = {}, options = {}) {
  ...
  const client = options.client || input.client || createMockClient(options.mock || input.mock || 'pass');
  const routes = [];
  ...
  for (const alias of aliases) {
    ...
    for (const checkName of checksToRun) {
      checks.push(await runSingleConformanceCheck(checkName, resolution, client, options));
    }
    routes.push({
      requestedAlias: alias,
